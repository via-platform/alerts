...
module.exports = class AlertCreator {
    static deserialize({manager, omnibar}, state){
        return new AlertCreator({manager, omnibar, uri: state.uri, advanced: this.advanced});
    }
    ...
    getURI(){
        return base + (this.alert.market ? '/' + this.alert.market.exchange.id + '/' + this.alert.market.symbol : '');
    }
