...
class AlertsPackage {
    ...
    message(msg){
        const {sequence, changes} = JSON.parse(msg);

        //TODO Check the message sequence
        
        if(changes.type === 'snapshot'){
            for(const alert of this.open()) alert.destroy();
            for(const alert of changes.alerts) this.update(alert, false);
        }else if(changes.type === 'canceled'){
            const alert = this.find(changes.uuid);

            if(alert){
                return alert.destroy();
            }else{
                //No clue what happened if it hits this line...
                via.console.warn(`Received message to update unknown alert.`, changes);
                console.warn('Alert not found:', changes);
            }
        }else if(changes.type === 'updated'){
            this.update(changes.alert);
        }else if(changes.type === 'created'){
            this.update(changes.alert);
        }else if(changes.type === 'triggered'){
            const alert = this.find(changes.event.alert);

            if(alert){
                alert.trigger(changes.event);
            }else{
                //No clue what happened if it hits this line...
                via.console.warn(`Received changes to update unknown alert.`, changes);
                console.warn('Alert not found:', changes);
            }
        }
    }
